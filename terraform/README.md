...
This directory contains Infrastructure as Code (IaC) for deploying the Wrench system using
Terraform.
...
│       ├── versions.tf
│       └── terraform.tfvars     # (gitignored) fresh secrets, deployment_name=prod-dev
└── README.md                    # This file
```
...
2. **Create R2 Bucket** for Terraform state:
   - Bucket name: `wrench-terraform-state`
...
curl https://wrench-control-plane-prod.<subdomain>.workers.dev/health
...
curl https://wrench-control-plane-prod.<subdomain>.workers.dev/sessions
```
...
3. Check logs: `modal app logs wrench`

### Worker deployment fails

1. Build workers first: `npm run build -w @wrench/control-plane`
...
The live deployment. `deployment_name = "prod"`.

- Control plane: `wrench-control-plane-prod.mitchstahl11.workers.dev`
- Web app: `wrench-prod.vercel.app`
- Modal app: `wrench`

### Dev (`environments/dev/`)

A parallel deployment for safe iteration. `deployment_name = "prod-dev"`.

- Control plane: `wrench-control-plane-prod-dev.mitchstahl11.workers.dev`
- Modal app: `wrench-dev`
...
| `npm run deploy:shared`    | Build `@wrench/shared` only             |
